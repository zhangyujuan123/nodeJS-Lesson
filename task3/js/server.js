...
http.createServer(function(req,res){
    ...
    else if(path.indexOf('/getDetail')>=0){
        showContent(req,res);
    }
    else if(path=='/login'){
        showHtml(res,'login.html');
    }
    else if(path=='/listmanager'&&req.method=='GET'){
        showHtml(res,'list.html');
    }
    else if(path=='/listmanager'&&req.method=='POST'){
        checkUser(req,res);
    }
    else if(path=='/addChapter'){
        showHtml(res,'addChapter.html');
    }
    else if(path=='/login_bg.jpg'){
    ...
    else if(path=='/addlist'){
        showList(res);
    }
}).listen(8083);
function showHtml(res,html){
    var indexPath=path.join(__dirname,'../'+html);
    var fileContent=fs.readFileSync(indexPath);
    res.writeHead(200,{"Content-Type":"text/html"});
    res.write(fileContent);
    res.end();
}
function showImg(res,png){
    var imgPath=path.join(__dirname,'..'+png);
    var imgContent=fs.readFileSync(imgPath);
    res.writeHead(200,{"Content-Type":"image/jpg"});
    res.write(imgContent);
    res.end();
}
function showCss(res,css){
...
function showContent(req,res){
    var urlObj=url.parse(req.url);
    var cha=urlObj.query.split("=");
    for(var i=0;i<chapterList.length;i++){
        if(chapterList[i].chapterId==cha[1]){
            var content=JSON.stringify(chapterList[i]);
            res.writeHead(200,{"Content-Type":"text/plain"});
            res.end(content);
        }
    }
...
function addContent(req,res){
    var dataStr="";
        ...
        req.on("end",function(){
            ...
            var data={};
            data.chapterName=dataObj.title;
            data.chapterContent=dataObj.content;
            data.chapterId=chapterList.length+1;
